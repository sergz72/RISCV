...
/*********************************************************************
 * @fn      ethernet_packet_received_handler
 *
 ...
 * @return  none.
 */
static void ethernet_packet_received_handler(void)
{
    ...
    while(!(pDMARxSet->Status & ETH_DMARxDesc_OWN))
    {
        ...
        (pDMARxSet->Status & ETH_DMARxDesc_FS) )
        {
            ...
            ethernet_packet_received((void*)buffer, length);
        }
...
void WCHNET_ETHIsr(void)
{
    ...
    if( int_sta & ETH_DMA_IT_NIS )
    {
        if( int_sta & ETH_DMA_IT_R )
        {
            ...
            ethernet_packet_received_handler();
            ETH_LedDataSet(LED_OFF);
