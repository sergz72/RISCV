...
#define DEF_UART_RX_TIMEOUT     30

const __attribute__ ((aligned(4))) uint8_t Com_Cfg[ 8 ] = {
...
volatile int CDC_Tx_InProgress;
...
void USBHS_Device_Endp_Init ( void )
{
    ...
    USBHSD->UEP3_RX_CTRL = USBHS_UEP_R_RES_STALL;
            ...
            if( USBHS_SetupReqType & USB_REQ_TYP_CLASS )
            {
                ...
                switch( USBHS_SetupReqCode )
                {
                    case CDC_GET_LINE_CODING:
                        pUSBHS_Descr = (uint8_t *)&Com_Cfg[ 0 ];
                        len = 7;
                        break;
